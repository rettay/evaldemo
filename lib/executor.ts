...
import { getPack, getRulesForPack, getTarget } from "./db";
import type { ExecuteRunInput, ExecuteRunOutput, RunResult, Rule, Target } from "./types";
...
function renderTemplateDeep(obj: any, ctx: Record<string, string>) {
  ...
  if (typeof obj === "object") {
    const out: any = {};
    for (const [k, v] of Object.entries(obj)) out[k] = renderTemplateDeep(v, ctx);
    return out;
  }
  return obj;
}
...
function extractField(io: Rule["io"], output: any) {
  if (io.expects === "json") return output; // placeholder: add JSONPath later
...
function score(rule: Rule, actual: any): RunResult {
  switch (rule.scoring) {
    case "json_schema": {
      const r = scoreJsonSchema(rule.expected, actual);
      return { ruleId: rule.id, status: r.passed ? "pass" : "fail", score: r.score, details: r.details };
    }
    case "exact": {
      const want = String(rule.expected?.exact ?? "");
      const got = String(actual ?? "");
      const ok = want === got;
      return { ruleId: rule.id, status: ok ? "pass" : "fail", score: ok ? 1 : 0, details: ok ? undefined : { want, got } };
    }
    default: {
      return { ruleId: rule.id, status: "error", score: 0, details: { error: `Unknown scorer: ${rule.scoring}` } };
    }
  }
}
...
export async function executeRun(input: ExecuteRunInput): Promise<ExecuteRunOutput> {
  ...
  for (const rule of rules) {
    try {
      ...
      const scored = score(rule, actual);
      results.push(scored);
    } catch (err: any) {
      results.push({ ruleId: rule.id, status: "error", score: 0, details: { error: String(err?.message || err) } });
    }
  }
